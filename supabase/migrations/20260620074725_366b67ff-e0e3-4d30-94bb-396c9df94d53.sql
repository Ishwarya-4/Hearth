...
-- profiles: anyone authenticated can read profiles (needed to show avatars of co-members); only self can edit
CREATE POLICY "profiles_select_all" ON public.profiles FOR SELECT TO authenticated USING (true);
CREATE POLICY "profiles_insert_self" ON public.profiles FOR INSERT TO authenticated WITH CHECK (auth.uid() = id);
CREATE POLICY "profiles_update_self" ON public.profiles FOR UPDATE TO authenticated USING (auth.uid() = id);

-- calendars: members can view; owners can update/delete; any authenticated user can create
...
-- calendar_members: members can see member list; owners can add/remove
...
-- events: members can view; editors/owners can write
...
-- invitations: calendar owner can manage; invited user can see by email
...
-- When a calendar is created, also add the owner as a member (in case the app forgets)
